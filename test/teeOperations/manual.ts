...
import type { TEEManualTestSuite } from '../../types/contracts/tests/TEEManualTestSuite';
import { createInstances } from '../instance';
import { getSigners, initSigners } from '../signers';

async function deployTfheManualTestFixture(): Promise<TEEManualTestSuite> {
    const signers = await getSigners();
    const admin = signers.alice;

    const contractFactory = await ethers.getContractFactory('TEEManualTestSuite');
...
describe('TEE manual operations', function () {
    ...
    //     const res = await this.contract.test_select_eaddress(false, input, input2);
    ...
    //     const res = await this.contract.test_select_eaddress(true, input, input2);
